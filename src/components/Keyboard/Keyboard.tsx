...
const Keyboard = ({}: KeyboardProps) => {
  ...
  const handleClick: React.MouseEventHandler<HTMLDivElement> = (event) => {
    ...
    if (
      state.word.toLowerCase().includes(currentLetter) &&
      !clickedKeys.includes(currentLetter)
    ) {
      ...
      <article className="Keyboard">
        <section className="key-section">
          {keys.map((key) => {
            ...
            return (
              <div
                onMouseEnter={isClicked ? undefined : handleMouseEnter}
                id={key}
                onClick={isClicked ? undefined : handleClick}
                key={key}
                className={`${isClicked ? "clicked" : "key-box"}`}
              >
                {key}
              </div>
            );
